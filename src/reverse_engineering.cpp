...
void dump_address_space(int begin, int end)
{
    for (uint16_t i = begin; i <= end; ++i)
    {
        ...
        while (b1 != b2 && timeout)
        {
            ...
            {
                /*
                 * Some addresses are used as RAM by the ECU, so some values keep changing. 
                 * If the value changes more than 3 times, then we treat this as RAM and send one of the values.
                 */
                --timeout;
...
void monitor_special_addresses(int force_one)
{
    ...
    const char *string_array[] PROGMEM = 
    {
        "bitvector                   ",
        "potential speed             ",
